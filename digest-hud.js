(function () {
    ...
    angular.module('digestHud', []).provider('digestHud', ['$provide', function ($provide) {
        ...
        this.enable = function () {
            ...
            buttonsElement.on('click', function (ev) {
                ev.stopPropagation();
            });

            hudElement.on('mousedown mouseup click', function (ev) {
                ev.stopPropagation();
            });
            hudElement.css({
                position: 'fixed',
            ...
            $provide.decorator('$rootScope', ['$delegate', function ($delegate) {
                ...
                function instrumentedDigest() {
                    ...
                    var lastLimit = 500;
                    var lastNum = digestTimings.reduce(function (num, e) {
                        return start - e.start <= lastLimit ? num + 1 : num;
                    }, 0);
                    var description =
                        angular.version.full + ' ' + (toggle ? '\u25cf' : '\u25cb') + ' ' + digestTimings.length + ' stats [min:' + sorted[0] + 'ms,med:' +
                        median + 'ms,max:' + sorted[len - 1] + 'ms,last:' + duration + 'ms] ' + lastNum + " for last " + lastLimit + 'ms';
                    if (digestHud.logStatsEnabled) {
        ...
        this.setHudPosition = function (position) {
            if (hudElement) {
                ...
                position.split(' ').map(function (prop) {
                    styles[prop] = 0;
                });
    ...
    angular.element(function () {
        ...
        var app_name = html.getAttribute('digest-hud-lazy-ng-app');
        if (!app_name) {
            return;
        }
        html.setAttribute('data-ng-app', app_name);
        var app = angular.module(app_name);
        app.requires.push('digestHud');
        app.config(['digestHudProvider', function (digestHudProvider) {
            ...
            digestHudProvider.numTopWatches = parseInt(html.getAttribute('digest-hud-top-watches')) || 10;  // number of items to display in detailed table
            digestHudProvider.numDigestStats = parseInt(html.getAttribute('digest-hud-digest-cycles')) || 5;  // number of most recent digests to use for min/med/max stats
            digestHudProvider.logStatsEnabled = html.getAttribute('digest-hud-log-stats')=='true';
        }]);
